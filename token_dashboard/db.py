...
def _migrate_add_source(conn) -> None:
    """Add source column to messages and tool_calls for opencode support.

    Why: opencode support requires distinguishing transcripts from claude-code
    vs opencode. Existing rows are Claude Code data and default to 'claude'.
    How to apply: if the table exists without the column, add it with a
    default of 'claude'. The source of truth is on disk; we do not clear rows
    because this migration only adds metadata and backfills safely.
    """
    for table in ("messages", "tool_calls"):
        ...
        if "source" in cols:
            continue
        conn.execute(f"ALTER TABLE {table} ADD COLUMN source TEXT DEFAULT 'claude'")
    conn.commit()
